...
# ログフォーマット例: [2025/12/17 16:33:58] INFO: Message
...
ARCHIVE_FILE_PATTERN = re.compile(
    rf'^{re.escape(NX_JIKKYO_SERVER_LOG_PATH.stem)}\.(\d{{8}}){re.escape(NX_JIKKYO_SERVER_LOG_PATH.suffix)}$',
)
...
def GetArchiveFilePath(date_key: str) -> Path:
    ...
    Returns:
        Path: アーカイブファイルのパス（例: logs/archives/NX-Jikkyo-Server.20260212.log）
    """

    # アーカイブ命名規則を 1 箇所に集約して、生成と解析の不一致を防ぐ
    ## 例: NX-Jikkyo-Server.log -> NX-Jikkyo-Server.20260212.log
    return LOGS_ARCHIVES_DIR / f'{NX_JIKKYO_SERVER_LOG_PATH.stem}.{date_key}{NX_JIKKYO_SERVER_LOG_PATH.suffix}'
...
def CleanupOldArchiveLogs(retention_days: int | None) -> None:
    ...
    for archive_path in LOGS_ARCHIVES_DIR.glob(f'{NX_JIKKYO_SERVER_LOG_PATH.stem}.*{NX_JIKKYO_SERVER_LOG_PATH.suffix}'):
...
def SplitServerLogByDate() -> None:
    ...
    try:
        ...
        # ログファイルが存在しない場合は分割不要
        if NX_JIKKYO_SERVER_LOG_PATH.exists() is False:
            return

        # ファイルサイズの取得を試みる
        ## サイズ取得に失敗する環境（権限不足・I/O エラー等）では分割処理を諦めて起動継続を優先する
        try:
            file_size = NX_JIKKYO_SERVER_LOG_PATH.stat().st_size
        ...
        try:
            with NX_JIKKYO_SERVER_LOG_PATH.open(encoding='utf-8', errors='replace') as source_file:
        ...
        try:
            ...
            with ExitStack() as exit_stack:
                ...
                source_file = exit_stack.enter_context(
                    NX_JIKKYO_SERVER_LOG_PATH.open(encoding='utf-8', errors='replace'),
                )
                ...
                today_temp_file = exit_stack.enter_context(
                    tempfile.NamedTemporaryFile(
                        mode='w',
                        encoding='utf-8',
                        dir=NX_JIKKYO_SERVER_LOG_PATH.parent,
                        prefix='.NX-Jikkyo-Server.today.',
                        suffix='.log.tmp',
                        delete=False,
                    ),
                )
            ...
            if today_temp_path is not None:
                today_temp_path.replace(NX_JIKKYO_SERVER_LOG_PATH)
